...
def test_night_resolver_kill_no_heal() -> None:
    ...
    state = GameState(
        ...
        players=[
            PlayerState(user_id=p1_id, telegram_id=1, display_name="P1", role=RoleId.MAFIA.value),
            PlayerState(user_id=p2_id, telegram_id=2, display_name="P2", role=RoleId.CIVILIAN.value),
            PlayerState(user_id=p3_id, telegram_id=3, display_name="P3", role=RoleId.DOCTOR.value),
        ]
    )
...
def test_night_resolver_sheriff_check() -> None:
    ...
    state = GameState(
        ...
        telegram_chat_id=123,
        players=[
            PlayerState(user_id=p1_id, telegram_id=1, display_name="P1", role=RoleId.SHERIFF.value),
            PlayerState(user_id=p2_id, telegram_id=2, display_name="P2", role=RoleId.MAFIA.value),
        ]
    )
