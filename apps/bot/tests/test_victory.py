...
def test_victory_maniac_does_not_win_against_two_town() -> None:
    state = GameState(
        game_id=uuid4(),
        chat_id=uuid4(),
        telegram_chat_id=123,
...
def test_victory_unknown_role_ignored() -> None:
    state = GameState(
        game_id=uuid4(),
        chat_id=uuid4(),
        telegram_chat_id=123,
    ...
    result = VictoryConditionService.check(state)
    # 1 town alive, nothing else -> civilians win
    assert result.winner_side == WinnerSide.CIVILIANS


def test_victory_none_role_ignored() -> None:
    state = GameState(
        game_id=uuid4(),
        chat_id=uuid4(),
        telegram_chat_id=123,
...
def test_victory_civilians_winning_ids_include_dead_town() -> None:
    p1 = uuid4()
    p2 = uuid4()
    state = GameState(
        game_id=uuid4(),
    ...
    result = VictoryConditionService.check(state)
    assert result.winner_side == WinnerSide.CIVILIANS
    assert p1 in result.winning_player_ids
    assert p2 in result.winning_player_ids


def test_victory_mafia_winning_ids_include_dead_mafia() -> None:
    p1 = uuid4()
    p2 = uuid4()
    state = GameState(
        game_id=uuid4(),
